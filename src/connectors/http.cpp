...
namespace connectors {
	...
	http::http(const boost::shared_ptr<logger> & l, bool verifyHost)
		: 	connector(l),
			/*context_(boost::asio::ssl::context::no_compression),*/
			resolver_(service_),
			...
			answered_(false)
	{
		...
		if (verifyHost)
		{
			...
			/*socket_.set_verify_callback(
				boost::bind(&http::handle_checkCertificate, this, _1, _2));*/
		}
		else
		{
			//socket_.set_verify_mode(boost::asio::ssl::verify_none);
	...
	void http::setHost(const std::string & host, const std::string & port)
	{
	...
	void http::setQuery(const boost::shared_ptr<query> & qr)
	{
	...
	std::stringstream & http::getStream()
	{
	...
	void http::unckunck(boost::asio::streambuf & buf)
	{
		bool start = true;
		std::string c;
	...
	// callbacks
	bool http::handle_checkCertificate(bool preverified, boost::asio::ssl::verify_context& ctx)
	{
	...
	void http::handle_resolve(const boost::system::error_code& err,
						boost::asio::ip::tcp::resolver::iterator endpoint_iterator)
	{
		if (!err)
		{
			...
			boost::asio::async_connect(socket_.lowest_layer(), endpoint_iterator,
				boost::bind(&http::handle_connect, this,
					boost::asio::placeholders::error));
		}
		else
		{
	...
	void http::handle_connect(const boost::system::error_code& err)
	{
		if (!err)
		{
			boost::asio::async_write(socket_, request_,
				boost::bind(&http::handle_write_request, this,
					boost::asio::placeholders::error,
	...
	/*void http::handle_handshake(const boost::system::error_code& err)
	{
		if (!err)
		{
	...
	void http::handle_write_request(const boost::system::error_code& err, size_t bytes_transferred)
	{
		...
		if (!err)
		{
			...
			boost::asio::async_read_until(socket_, response_, "\r\n",
				boost::bind(&http::handle_read_status_line, this,
					boost::asio::placeholders::error,
					boost::asio::placeholders::bytes_transferred));
		}
		else
	...
	void http::handle_read_status_line(const boost::system::error_code& err, size_t bytes_transferred)
	{
		if (!err)
		{
			...
			if (status_code == 200)
			{
				...
				boost::asio::async_read_until(socket_, response_, "\r\n\r\n",
					boost::bind(&http::handle_read_headers, this,
						boost::asio::placeholders::error,
						boost::asio::placeholders::bytes_transferred));
			}
			else if (status_code == 302)							// redirection
			{
				// Read the response headers, which are terminated by a blank line.
				boost::asio::async_read_until(socket_, response_, "\r\n\r\n",
					boost::bind(&http::handle_redirection, this,
						boost::asio::placeholders::error,
						boost::asio::placeholders::bytes_transferred));
			}
			else
	...
	void http::handle_redirection(const boost::system::error_code& err, size_t bytes_transferred)
	{
		if (!err)
		{
			...
			condition_.notify_one(); answered_ = false;
		}
		else
		{
			logger_->add("Error: " + err.message() + "\n",
	...
	void http::handle_read_headers(const boost::system::error_code& err, size_t bytes_transferred)
	{
		if (!err)
		{
			// Process the response headers.
			...
			boost::asio::async_read(socket_, response_,
				...
				boost::bind(&http::handle_read_content, this,
					boost::asio::placeholders::error,
					boost::asio::placeholders::bytes_transferred));
		}
		else
	...
	void http::handle_read_content(const boost::system::error_code& err, size_t bytes_transferred)
	{
		if (!err)
		{
